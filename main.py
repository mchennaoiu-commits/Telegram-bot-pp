...
stopuser = {}
token = "8559122495:AAECSUbF4gRclR8o_CfnnZKN-Z2rj2Cdrtw" #توكنك هنا
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='SIMO-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
...
طريقة الاستخدام : فقط أضغط على الكود و سيتم نسخه تلقائياً و ادخل الى البوت @Ufpschkbot و ارسل الكود الذي نسختة
</b>'''
			bot.reply_to(message,msg,parse_mode="HTML")
		except Exception as e:
			print('ERROR : ',e)
